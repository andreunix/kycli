...
import { KYCLI_EXTENSION } from '../arguments/extension.js'
...
export async function getConfig(
	args: GetConfigArgs,
): Promise<ResolvedKycliConfig | null> {
	...
	const config = await loadKycliConfig(configFile, jiti)
...
export async function getConfigOrFail(
	args: GetConfigArgs,
): Promise<ResolvedKycliConfig> {
	const config = await getConfig(args)

	if (!configFileExists(config)) {
		throw new Error(
			'No config file found. Try creating one by running `kycli init`.',
		)
	}
...
export function configFileExists(
	config: ResolvedKycliConfig | null,
): config is ResolvedKycliConfig {
	if (!config) {
		return false
	}
	...
	return (
		...
		configFile !== 'kycli.config' &&
		configFile !== 'kysely.config'
	)
}

const BASE_CONFIG_FILESNAMES = [
	`kycli.config.${KYCLI_EXTENSION}`,
	`kysely.config.${KYCLI_EXTENSION}`,
]
...
async function findNearestKyselyConfigPath(): Promise<string | null> {
	...
	consola.debug('found kycli config file at', configFile)
...
async function loadKycliConfig(
	filePath: string,
