...
config: _omitted0, environment: _omitted1, 'no-filesystem-caching': _omitted3, ...CommonArgsForInit } = CommonArgs;
...
const Command = defineCommand(args, {
    meta: {
        description: 'Create a sample `kycli.config` file',
    },
    async run(context) {
        const { args } = context;
        const config = await getConfig(args);
        if (configFileExists(config)) {
            return consola.warn(`Init skipped: config file already exists at ${config.configMetadata.configFile}`);
        }
        const filePath = join(getCWD(), `kycli.config.${KYCLI_EXTENSION}`);
