...
import { getConsumerPackageJSON, getCTLPackageJSON, } from './pkg-json.js';
/**
 ...
 */
export async function getCTLInstalledVersion() {
    try {
        const pkgJSON = await getCTLPackageJSON();
        return getVersionFromPackageJSON('kycli', pkgJSON);
    }
    catch {
        return null;
    }
 ...
 */
export async function printInstalledVersions() {
...
export async function printUpgradeNotice(args) {
    ...
    const command = detectedPackageManager.name === packageManagerName
    ...
    consola.box(notices
        .map(([prettyName, name, installedVersion, latestVersion]) => `A new ${prettyName} version is available: ${installedVersion ? `v${installedVersion}` : '[not installed]'} -> v${latestVersion}\nRun \`${command} ${name === 'kycli'
        ? installGloballyCommand(name)
        : installLocallyCommand(name)}\` to upgrade.`)
        .join('\n\n'));
}
async function getVersionFromCatalog(packageName, catalogReference) {
    ...
    const rawWorkspaceFile = await readOptionalFile(join(workspaceDirPath, 'pnpm-workspace.yaml'));
    if (rawWorkspaceFile) {
    ...
    if (rawPackageJson) {
        const rootPkgJSON = JSON.parse(rawPackageJson);
...
async function findWorkspaceDir(startingFrom) {
...
function parsePnpmWorkspaceCatalog(raw) {
    const catalog = {};
    const catalogs = {};
