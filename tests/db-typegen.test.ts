...
import { Kysely, DummyDriver, PostgresAdapter, PostgresQueryCompiler } from "kysely";
import { runTypegen } from "../src/typegen/typegen";
import type { ResolvedKycliConfig } from "../src/config/kycli-config";
...
async function createTempProject() {
  const cwd = await mkdtemp(join(tmpdir(), "kycli-typegen-"))
  ...
  } as ResolvedKycliConfig

  return { config, cwd, migrationsFolder }
}

describe("db typegen", () => {
  test("generates files from migrations and centralizes kysely imports", async () => {
  ...
  test("omits Generated import when no generated columns are present", async () => {
    ...
    expect(types).not.toContain('import type { Generated } from "kysely";')
